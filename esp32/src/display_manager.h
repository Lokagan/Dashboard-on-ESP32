...
// ============================================================
// DISPLAY_MANAGER.H — LVGL 9.x + TFT_eSPI (ILI9341)
// ============================================================
...
// Buffer de dessin LVGL, en RAM interne. ⚠️ 2 o/px, JAMAIS sizeof(lv_color_t) :
// en LVGL 9 ce type est un struct RGB888 de 3 o, indépendant du format
// d'affichage (RGB565_SWAPPED ici). Le calcul d'origine multipliait par 3 — on
// allouait 11 520 o en croyant en poser 7 680, et LVGL, qui découpe selon le
// VRAI format, en tirait 18 lignes. Le log annonçait « 12 lignes » pendant que
// les traces montraient 6 flush pour 112 lignes (18,7 par bande).
//
// 18 lignes assumées : la marge d'interne le permet (pire cas mesuré ~21 Ko
// après échange vocal), et c'est autant de flushes en moins. Repasser à 12
// rendrait 3 840 o — à réévaluer si l'écran Table fait plonger le pire cas.
// Second consommateur : la page MEMOIRE de SysInfo (_si_alloc).
#define LV_BUF_LINES 18
#define LV_BUF_BYTES (SCREEN_WIDTH * LV_BUF_LINES * 2)

// Buffers JSON des tables, alloués en PSRAM (11 Ko d'interne rendus aux piles
// d'ESP_SR). Restent nullptr si l'allocation échoue → tous les accès sont
// gardés. Second consommateur : _si_alloc.
...
// Coupe/reprend LVGL. À utiliser autour de toute écriture directe sur _tft
...
// --- Utils ---
// Diagnostic (cmd "spy") — journalise les `count` prochaines invalidations LVGL,
// rectangles AVANT fusion, puis se désarme. Identifie QUI redessine l'écran.
...
// Progression et échec OTA — dessinés en TFT_eSPI DIRECT (hors LVGL), pour
// rester visibles alors que LVGL est en pause pendant le transfert.
